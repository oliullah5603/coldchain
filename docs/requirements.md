...
| Write audit records using AuditEvent and prevent tampering **(partial)** | Remote FHIR AuditEvent plus local AuditEvent-shaped HMAC chain, SQL mutation-blocking triggers, verification command, and dispatch lock when verification fails. Literal tamper-proof retention is not fulfilled by this free demo; the chain is tamper-evident. | `src/store.js`; mutation and tampering tests |
